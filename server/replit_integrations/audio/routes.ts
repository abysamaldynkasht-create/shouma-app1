...
export function registerAudioRoutes(app: Express): void {
  ...
  app.post("/api/conversations/:id/messages", audioBodyParser, async (req: Request, res: Response) => {
    try {
      ...
      res.on("error", (err) => {
        console.error("Audio integrations route connection error:", err);
        isClosed = true;
      });
      ...
      try {
        ...
        console.error("Error writing user_transcript chunk:", err);
        isClosed = true;
      }
      ...
      for await (const chunk of stream) {
        ...
        try {
          ...
          console.error("Error writing Integration Audio stream chunk:", err);
          break;
        }
      }
      ...
      if (!isClosed) {
        try {
          ...
          console.error("Error ending Integration Audio stream:", err);
        }
      }
    } catch (error) {

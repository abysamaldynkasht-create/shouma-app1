...
import { 
  Home, Compass, LogOut, Check, X, 
  Send, ShieldCheck, MapPin, Sparkles, MessageSquare, AlertCircle, Phone, Calendar,
  CreditCard, Landmark, Loader2, User
} from 'lucide-react';
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
                  ...
                  <div className="text-2xl font-black text-white mt-1">
                    {trips.filter(t => t.status === 'assigned').length || 0} طلبات
                  </div>
                  <span className="text-[10px] text-amber-500 font-medium mt-1 block font-sans">تحتاج إلى موافقة أو رفض</span>
                </div>
                <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
                  ...
                  <span className="text-[10px] text-slate-500 font-medium mt-1 block font-sans">تخصم الأتعاب نهاية كل أسبوع</span>
                </div>
                <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl">
                  ...
                  <span className="text-[10px] text-slate-500 font-medium mt-1 block font-sans">من أصل ٢٢ سائح دولي</span>
                </div>
              </div>
              ...
              <div className="bg-slate-900/40 border border-slate-800/80 rounded-2xl p-6">
                <h3 className="text-lg font-bold text-white mb-2 flex items-center gap-1.5 mb-4">
                  ...
                  <span>تلميحات وإرشادات مهمة للمرشدين السياحيين:</span>
                </h3>
            ...
            >
              ...
              {trips.length === 0 ? (
                <div className="p-12 text-center bg-slate-900 border border-slate-800 rounded-3xl">
                  <Compass className="w-12 h-12 text-slate-600 mx-auto mb-3 animate-spin" style={{ animationDuration: '4s' }} />
                      ...
                      <div className="flex justify-between items-start mb-4 border-b border-slate-800 pb-3">
                        ...
                        <div>
                          ...
                          {trip.status === 'completed' && (
                            <span className="text-[10px] bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 px-2 py-1 rounded-full font-bold">مكتملة</span>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="space-y-2.5 text-xs text-slate-300 md:text-sm mb-5">
                        ...
                        {trip.badges && Array.isArray(trip.badges) && trip.badges.length > 0 && (
                          <div className="mt-3.5 space-y-1.5">
                            <span className="text-[10px] text-amber-500 font-bold block">مربعات وميزات الرحلة المحددة:</span>
                            <div className="flex flex-wrap gap-1.5">
                              {trip.badges.map((badge: string, bIdx: number) => (
                                <span 
                                  key={bIdx} 
                                  className="inline-flex items-center gap-1.5 px-2 py-1 bg-slate-950 border border-slate-800 rounded-lg text-slate-300 font-semibold text-[10px]"
                                >
          ...
          {activeTab === 'account' && (
            ...
            >
              <div className="flex flex-col items-center text-center pb-6 border-b border-slate-800">
                <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-amber-500 to-orange-500 p-1 mb-4 shadow-xl animate-pulse">
                    ...
                    <div>
                      {!isLinkingFormOpen ? (
                        <div className="bg-slate-950/40 p-5 rounded-2xl border border-slate-800/80 text-center">
                          <div className="w-12 h-12 bg-red-500/10 rounded-full flex items-center justify-center mx-auto mb-3 text-red-400 border border-red-500/20">

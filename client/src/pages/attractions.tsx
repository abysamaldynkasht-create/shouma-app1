...
export default function AttractionsPage() {
  ...
  const categories = useMemo(() => {
    const cats = new Set(combinedAttractions.map(a => a.category));
  ...
  const getCategoryLabel = (cat: string) => {
    ...
    const mapping: Record<string, { ar: string; en: string }> = {
      markets: { ar: "الأسواق التجارية", en: "Traditional Markets" },
      entertainment: { ar: "الأماكن الترفيهية", en: "Entertainment & Parks" },
      heritage: { ar: "الأماكن التراثية", en: "Heritage & History" },
      nature: { ar: "الأماكن الطبيعية", en: "Nature & Beaches" },
      wadis: { ar: "الأودية", en: "Wadis & Valleys" },
      springs: { ar: "العيون الطبيعية", en: "Natural Springs" },
      history: { ar: "قلاع وحصون تاريخية", en: "Historic Castles & Forts" },
      hiking: { ar: "مسير ومغامرة", en: "Hiking & Adventures" },
      beach: { ar: "شواطئ وجزر", en: "Beaches & Islands" }
    };
    if (mapping[cat]) {
      return isAr ? mapping[cat].ar : mapping[cat].en;
    }
    return cat;
  };
  ...
  }, [combinedAttractions, searchQuery, selectedGovernorate, selectedWilayat, selectedCategory]);
      ...
      <section className="relative h-[40vh] overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center px-4 text-center">
          ...
          <p className="text-lg text-white/90 max-w-2xl mb-4 drop-shadow" data-testid="text-page-subtitle">
            {t('moreThanPlaces')} {combinedAttractions.length} {t('placesIn')} {governorates.length} {t('governorates')}
          </p>
        </div>
      </section>

...
export default function HotelsPage() {
    ...
    fetch('/api/catalog/hotels')
      .then(async r => {
        ...
        if (text.trim().startsWith("<")) {
          console.warn("Received HTML instead of JSON for hotels catalog.");
          return [];
        }
        try {
  ...
  const combinedHotels = useMemo(() => {
    const formatted = dbHotels.map(item => ({
      ...
      amenities: Array.isArray(item.amenities) && item.amenities.length > 0 ? item.amenities : ["واي فاي", "موقف سيارات", "مسبح", "تكييف"],

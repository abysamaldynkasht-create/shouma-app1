...
export default function ActivitiesPage() {
  ...
  const { data: dbActivities = [], isLoading } = useQuery<any[]>({
    queryKey: ["/api/catalog/activities"],
    queryFn: async () => {
      const resp = await fetch("/api/catalog/activities");
      if (!resp.ok) return [];
      return resp.json();
    }
  });
  ...
  const mappedDbActivities = dbActivities.map((act) => ({
    id: `db-${act.id}`,
    name: act.name,
    nameAr: act.name_ar || act.nameAr,
    description: act.description,
    descriptionAr: act.description_ar || act.descriptionAr,
    location: act.location,
    region: act.region,
    duration: act.duration,
    price: act.price,
    image: act.image,
    rating: parseFloat(act.rating) || 4.8,
    includes: Array.isArray(act.includes) ? act.includes : [],
    provider: act.provider,
    phone: act.phone,
    mapUrl: act.map_url || act.mapUrl,
    branches: typeof act.branches === 'string' ? JSON.parse(act.branches) : act.branches,
  }));

  const allActivities = [...mappedDbActivities, ...activities];
